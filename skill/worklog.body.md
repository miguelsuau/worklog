...
9. When the substantive tracked task is clearly complete, run the Session Log Review workflow automatically and present the rendered session-log draft in the same completion response. Do not merely offer to create it later. Also run the workflow when the user explicitly asks to log or review the session, or when the agent must stop and preserve reviewed state. If the task may reasonably continue, ask the user before drafting a session log instead of treating a substep or idle moment as session completion. Do not prompt for session-log approval when the only activity was Worklog project setup/configuration, template or sharing configuration, status inspection, resume-context loading, or log review/approval.
...
Start this workflow when one of these timing gates is true: the tracked task is clearly complete, the user explicitly asks to log or review the session, or the agent must stop and preserve reviewed state. A clearly complete substantive Worklog-tracked task is a mandatory logging boundary: draft and present the session log automatically in the same response that reports completion. If the user may reasonably intend to continue the task, ask whether to draft the session log now instead of drafting it proactively. When calling `worklog_draft_session_log`, pass `review_reason` as `task_complete`, `user_requested_log_review`, or `agent_must_stop` to match the timing gate.
...
- When a substantive Worklog-tracked task is clearly complete, draft and present the session log automatically in the same response that reports completion. Do not merely offer to create the session log later.
