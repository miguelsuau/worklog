...
11. For shared projects, do not combine provider selection and path or connector-target selection. The workflow is: suggest providers, user selects provider, Worklog builds/inspects that provider backend, Worklog suggests paths or connector targets for that provider, user selects path or target, then configure sharing. If the storage provider is missing, ask a direct provider question even if the user has approved a path or target. Never infer the provider from an example path, mounted folder name, ticketing-system name, or "path looks good." When the user selects a path or connector target in response to Worklog's prompt, treat that selection as the sharing confirmation: call the tool immediately with `confirmed_by_user: true` and `confirmation_quote` set to the user's exact selection message. Do not ask the user to repeat a boilerplate confirmation sentence.
