...
CLAUDE_COMMAND_PATH = ROOT / "packages" / "claude" / "commands" / "worklog.md"
...
def render_skill(host: dict[str, Any], body: str) -> str:
    return (
        ...
        "---\n\n"
        "<!-- Generated from skill/worklog.body.md by scripts/generate_package_files.py. "
...
def render_claude_command(body: str) -> str:
    invocation_note = (
        "Claude can invoke this command with `/worklog`. `$worklog`, `@worklog`, "
        "and `\\worklog` are command-like conventions from other agent hosts; if "
        "the host passes any of them through in the prompt, treat it exactly like "
        "an explicit Worklog invocation."
    )
    description = (
        "Use Worklog when the user invokes /worklog, asks to use Worklog, or wants "
        "to track, resume, or review project work; create user-approved templates, "
    ...
    return (
        ...
        "Do not edit this file directly. -->\n\n"
        "# Worklog\n\n"
        f"{body.replace(PLACEHOLDER, invocation_note).rstrip()}\n"
    )
...
def generated_files() -> dict[Path, str]:
    ...
    outputs[CLAUDE_COMMAND_PATH] = render_claude_command(body)
